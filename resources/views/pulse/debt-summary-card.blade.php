<x-pulse::card :cols="$cols" :rows="$rows" :class="$class">
    <x-pulse::card-header name="Debt Summary">
        <x-slot:icon>
    ...
    <x-pulse::card-body>
        @if ($summary)
            <div class="flex items-center justify-between mb-4">
                <div class="text-3xl font-bold {{ match($summary['grade']) {
                    'A' => 'text-green-500',
                    'B' => 'text-cyan-500',
                    'C' => 'text-yellow-500',
                ...
                <div class="text-right text-sm text-gray-500 dark:text-gray-400">
                    <div>Score: <span class="font-semibold text-gray-800 dark:text-gray-200">{{ number_format($summary['score']) }}</span></div>
                    <div>Est. Hours: <span class="font-semibold text-gray-800 dark:text-gray-200">{{ number_format($summary['hours'], 1) }}h</span></div>
                    <div>Items: <span class="font-semibold text-gray-800 dark:text-gray-200">{{ number_format($summary['items']) }}</span></div>
                </div>
            </div>
